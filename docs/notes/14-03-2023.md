...
Now that we have created the btrfs subvolumes, it is time for the _readonly_ snapshot of the root subvolume.
...
Before we move on, we need to add the `neededForBoot = true;` to some mounted subvolumes in `hardware-configuration.nix`.
